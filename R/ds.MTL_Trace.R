...
ds.LS_MTL_Trace <- function (X, Y, lam, C, opts=list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=NULL,nRunsSensitAn=100), datasources=NULL, nDigits){
  ...
  while (iter < opts$maxIter){
    ...
    while (TRUE){
      wzp <- proximal_Trace(ws - Gws/gamma, lam / gamma);
...
ds.MTL_Trace_Train = function(X=NULL, Y=NULL, type="regress", nlambda=10, lam_ratio=0.1, lambda=NULL, C=0, 
  ...
  if (type=="regress"){
    ...
    for(i in 1:length(lam_seq)){
      m=ds.LS_MTL_Trace(X=X, Y=Y, lam=lam_seq[i], C=C, opts=optsTrain, datasources=datasources, nDigits=nDigits)
  ...
  #########################
    ...
    optsTrain=opts
    for(i in 1:length(lam_seq)){
      m=ds.LR_MTL_Trace(X=X, Y=Y, lam=lam_seq[i], C=C, opts=optsTrain, datasources=datasources, nDigits=nDigits)
      optsTrain$w0=m$W; optsTrain$init=1
      fit$ws[[i]]=m$W
      fit$Obj=c(fit$Obj, m$Obj)
      fit$Logs=rbind(fit$Logs, m$Logs)
...
ds.MTL_Trace_CVCroSite = function(X=NULL, Y=NULL, type="regress", lam_ratio=0.1, nlambda=10, lambda=NULL,
  ...
  if (type=="regress"){
    ...
    lam_seq=vector()
    for (k in 1:nTasks){
      fit=ds.MTL_Trace_Train(X=X, Y=Y, nlambda=nlambda, lam_ratio=lam_ratio, type="regress", opts=opts, C=C, lambda=lambda, 
    ...
    lam_seq=vector()
    for (k in 1:nTasks){
      fit=ds.MTL_Trace_Train(X=X, Y=Y, nlambda=nlambda, lam_ratio=lam_ratio, type="classify", opts=opts, C=C, lambda=lambda, 
                           datasources=datasources[-k], nDigits=nDigits)
...
ds.MTL_Trace_CVInSite = function(X=NULL, Y=NULL, type="regress", nfolds=10, lam_ratio=0.01, nlambda=10, lambda=NULL,
  ...
  if (type=="regress"){
    ...
    for (i in 1:length(cvPar)){
      ...
      fit=ds.MTL_Trace_Train(X="Xtrain", Y="Ytrain", nlambda=nlambda, lam_ratio=lam_ratio, type="regress", opts=opts, C=C, lambda=lambda, 
    ...
    for (i in 1:length(cvPar)){
      ...
      fit=ds.MTL_Trace_Train(X="Xtrain", Y="Ytrain", nlambda=nlambda, lam_ratio=lam_ratio, type="classify", opts=opts, C=C, lambda=lambda,
                           datasources=datasources, nDigits=nDigits)
      ws_task=lapply(1:nTasks, function(k) sapply(fit$ws, function(w)w[,k]))
