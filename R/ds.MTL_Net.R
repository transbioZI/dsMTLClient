...
#' @details Solver of FeMTL with least-square loss and network incorporation.  \cr A list of options controlling the optimization procedure and specifying the differential privacy component based on a privacy parameter epsilon>0 can be provided via the \code{opts} argument:
#' \itemize{
...
################################################################################
ds.LS_MTL_Net <- function (X, Y, lam, C, G, opts=list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=NULL,nRunsSensitAn=100), datasources=NULL, nDigits){
  ...
  LS_iter_update_tasks <- function(W){
    ...
    if(is.null(opts$diffPrivEpsilon)==FALSE){
      ...
      for(n in 1:nSimulations){
        ...
        grad_Ws_sensit[[n]]=grad_w_sensit + 2* C * W %*% GGt
        funcVals_sensit[[n]]=sum(funcVal_sensit) + C * norm(W %*% G, 'f')^2
      }
  ...
  LS_funcVal_eval_tasks <- function ( W){
    ...
    funcVal<-sum(unlist(func)) + C * norm(W %*% G, 'f')^2
...
#' 
#' @return The converged result of optimization
#' @details Solver of FeMTL with logistic loss and network incorporation. \cr A list of options controlling the optimization procedure and specifying the differential privacy component based on a privacy parameter epsilon>0 can be provided via the \code{opts} argument:
#' \itemize{
#' \item{\code{init} \cr A value (0 or 1) specifying the starting point of the involved gradient descent algorithm. Specifically, two options are provided: A value of 0 (default) uses the 0 matrix as starting point, while a value of 1 uses a starting point specified by the user. If applicable (i.e., \code{init=1}), the user-defined starting point (matrix) has to be specified via the \code{w0} argument in \code{opts}.}
#' \item{\code{w0} \cr A user-defined starting point (matrix) for the involved gradient descent algorithm, in case \code{init=1} is specified (otherwise, for \code{init=0}, the value is set to NULL, the default).}
#' \item{\code{tol} \cr A value >0 specifying the tolerance of the acceptable precision of solution to terminate the algorithm. Default value is set to 0.01.}
...
#' @author Han Cao, Roman Schefzik
################################################################################
ds.LR_MTL_Net <- function (X, Y, lam, C, G, opts=list(init=0,w0=NULL,tol=0.01,maxIter=50,ter=2,diffPrivEpsilon=NULL,nRunsSensitAn=100), datasources=NULL, nDigits){
